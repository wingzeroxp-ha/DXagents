...
function Set-GitBashEnvVar {
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            # Portable build: do not write user HERMES_GIT_BASH_PATH
...
function Main {
    ...
    [void](Test-Node)
    # Portable build: skip winget/choco/scoop system package installation
    Install-Repository
    Install-Venv
    Install-Dependencies
    Install-NodeDeps
    # Portable build: skip writing user PATH and HERMES_HOME
    Copy-ConfigTemplates
    # Portable build: skip interactive setup wizard
    Install-PlatformSdks
    # Portable build: do not auto-start gateway during packaging
    Write-Completion
}
